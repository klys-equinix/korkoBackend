...
describe('Coaches', () => {
  ...
  describe('/GET heartbeat', () => {
   ...
   describe('/POST coach', () => {
      it('should see that user exists', (done) => {
        chai.request(server)
            .post('/api/v1/coach/new')
            .send(testObject)
            .end((err, res) => {
    ...
    describe('/GET searchCategories', () => {
      it('should return one object', (done) => {
        chai.request(server)
            .get('/api/v1/coach/searchCategories').query({email: 'on@on.net',longitude:'20.48',lattitude:'52.2',radius:100,name:'on',subjects:'polish',level:['uniPrice','secondaryPrice'],lowEnd:0,highEnd:100,})
            .end((err, res) => {

                expect(res.body).to.be.an('object');
                expect(res.body).to.have.property('coaches').to.have.lengthOf(1);
                expect(res.body).to.have.deep.property('coaches[0].profile.email','on@on.net');
              done();
            });
      });
  });
   ...
   describe('/GET deleteOne', () => {
      it('should delete the user', (done) => {
        chai.request(server)
            .get('/api/v1/coach/deleteOne').query({email: 'on@on.net'})
            .end((err, res) => {
